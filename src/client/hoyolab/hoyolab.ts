...
export class Hoyolab {
   ...
   * @throws {HoyoAPIError} If `ltuid` or `ltoken` or `ltuidV2` or `ltokenV2` keys are missing in the `ICookie` object.
   *
   * @remarks
  ...
  public async gamesList(game?: GamesEnum): Promise<IGame[]> {
    ...
    this.request.setQueryParams({
      uid: this.cookie.ltuid || this.cookie.ltuidV2,
  ...
  async gameRecordCard(): Promise<IGameRecordCard> {
    ...
    this.request.setQueryParams({
      uid:
      this.cookie.ltuid || this.cookie.ltuidV2 || this.cookie.accountId || this.cookie.accountIdV2,
    })
